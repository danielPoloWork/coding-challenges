...
function buildConstellation(challenges) {
  ...
  for (let a = 0; a < nodes.length; a++) {
    for (let b = a + 1; b < nodes.length; b++) {
      const A = nodes[a].c, B = nodes[b].c;
      const shared = (A.patterns || []).filter((p) => (B.patterns || []).includes(p));
      if (shared.length) pEdges.push({ a, b, w: shared.length, shared });
      else if ((A.topics || []).some((t) => (B.topics || []).includes(t))) tEdges.push({ a, b });
    }
  }
  // neighbour lookup for hover highlighting (pattern links only)
  const nbr = nodes.map(() => new Set());
  pEdges.forEach((e) => { nbr[e.a].add(e.b); nbr[e.b].add(e.a); });
  return { nodes, pEdges, tEdges, nbr };
}
...
function Constellation({ challenges }) {
  ...
  useEffect(() => {
    ...
    function seed() {
      const cx = st.w / 2, cy = st.h / 2, R = Math.min(st.w, st.h) * 0.34;
      nodes.forEach((n, i) => {
    ...
    function tick() {
      const cx = st.w / 2, cy = st.h / 2, a = st.alpha;
      // pairwise repulsion
    ...
    function start() {
      ...
      if (reduced) { while (st.alpha > 0.02) tick(); draw(); }
      else { st.raf = requestAnimationFrame(loop); }
    }

    size(); seed(); st.alpha = 1;
